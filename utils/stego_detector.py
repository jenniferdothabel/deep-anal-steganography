...
def analyze_image_for_steganography(image_path):
    ...
    try:
        ...
        # Always convert to RGB - RGBA/palette/grayscale images all need 3 channels
        # to work correctly with the channel-based analysis functions
        img = img.convert('RGB')
...
def detect_lsb_steganography(pixels):
    ...
    r_bias = float(abs(np.mean(lsb_r) - 0.5) * 2)
    g_bias = float(abs(np.mean(lsb_g) - 0.5) * 2)
    b_bias = float(abs(np.mean(lsb_b) - 0.5) * 2)
    
    # Simplified run analysis for speed
    r_runs = float(min(count_runs(lsb_r[:10000]) / min(len(lsb_r), 10000), 1.0))  # Limit to 10k samples
    g_runs = float(min(count_runs(lsb_g[:10000]) / min(len(lsb_g), 10000), 1.0))
    b_runs = float(min(count_runs(lsb_b[:10000]) / min(len(lsb_b), 10000), 1.0))
    
    # Fast entropy calculation
    r_entropy = float(calculate_entropy(lsb_r[:5000]))  # Limit entropy calc to 5k samples
    g_entropy = float(calculate_entropy(lsb_g[:5000]))
    b_entropy = float(calculate_entropy(lsb_b[:5000]))
    
    # Also analyze the distribution of pairs of adjacent bits
    # This can detect more sophisticated steganography methods
    pair_analysis_r = float(analyze_bit_pairs(lsb_r[:5000]))  # Limit for speed
    pair_analysis_g = float(analyze_bit_pairs(lsb_g[:5000]))  # Limit for speed
    pair_analysis_b = float(analyze_bit_pairs(lsb_b[:5000]))  # Limit for speed
    ...
    r_correlation = float(np.corrcoef(lsb_r.flatten(), second_bit_plane_r.flatten())[0, 1])
    g_correlation = float(np.corrcoef(lsb_g.flatten(), second_bit_plane_g.flatten())[0, 1])
    b_correlation = float(np.corrcoef(lsb_b.flatten(), second_bit_plane_b.flatten())[0, 1])
    
    # Convert correlations to indicators (lower correlation = higher likelihood)
    r_corr_indicator = 1.0 - abs(r_correlation) if not np.isnan(r_correlation) else 0.5
    g_corr_indicator = 1.0 - abs(g_correlation) if not np.isnan(g_correlation) else 0.5
    b_corr_indicator = 1.0 - abs(b_correlation) if not np.isnan(b_correlation) else 0.5
    ...
    # Debug print for indicators
    # print(f"DEBUG LSB: bias={r_bias}, entropy={r_entropy_norm}, runs={r_runs}, pairs={pair_analysis_r}, corr={r_corr_indicator}")
    
    # Combined weights (Modified to be much more sensitive)
    r_indicator = (1.0 - r_bias) * 0.15 + r_entropy_norm * 0.1 + r_runs * 0.4 + pair_analysis_r * 0.2 + r_corr_indicator * 0.15
    g_indicator = (1.0 - g_bias) * 0.15 + g_entropy_norm * 0.1 + g_runs * 0.4 + pair_analysis_g * 0.2 + g_corr_indicator * 0.15
    b_indicator = (1.0 - b_bias) * 0.15 + b_entropy_norm * 0.1 + b_runs * 0.4 + pair_analysis_b * 0.2 + b_corr_indicator * 0.15
    
    # Use max instead of average to detect even single-channel manipulation
    lsb_likelihood = max(r_indicator, g_indicator, b_indicator)
    
    # Increase base value to ensure it's rarely zero for digital photos
    lsb_likelihood = max(lsb_likelihood, 0.05)
    
    # More aggressive scaling
    lsb_likelihood = scale_likelihood(lsb_likelihood, center=0.3, sensitivity=1.5)
    
    return float(lsb_likelihood)
...
def scale_likelihood(value, center=0.3, sensitivity=1.5):
    """
    Scale a likelihood value to make results more decisive and sensitive.
    """
    # Base floor for any digital image to avoid 0%
    if value < 0.01:
        return 0.0
        
    # Apply sigmoid-like scaling centered at the specified point
    # Lower center (0.3) makes it much more likely to show a percentage
    scaled = 1 / (1 + np.exp(-sensitivity * 5.0 * (value - center)))
    
    # Ensure it's at least a small percentage if indicators are present
    return max(0.01, min(1.0, float(scaled)))
